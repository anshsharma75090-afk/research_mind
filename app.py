...
def format_report_html(report: str) -> str:
    ...
    section_titles = {"introduction", "key findings", "conclusion", "sources"}
    ...
    for raw_line in report.splitlines():
        ...
        if normalized.startswith("research report:"):
            sections.append(("Title", [line.split(":", 1)[1].strip() or line]))
            continue
    ...
    title = next((lines[0] for heading, lines in sections if heading == "Title" and lines), "Research report")
    body_sections = [(heading, lines) for heading, lines in sections if heading != "Title"]

    section_html = ""
    for heading, lines in body_sections:
        if heading.lower() == "sources":
            ...
            for line in lines:
                ...
                if not clean_line:
                    continue
                source_items.append(f'<li>{linkify_text(clean_line)}</li>')
            content = f'<ul class="report-source-list">{"".join(source_items)}</ul>'
        else:
            paragraphs = []
            for line in lines:
                if re.match(r"^\d+\.\s+", line):
                    ...
                    paragraphs.append(
                        f'<div class="finding-row"><span>{html.escape(number)}</span>'
                        f'<p>{linkify_text(text.strip())}</p></div>'
                    )
                else:
                    paragraphs.append(f"<p>{linkify_text(line)}</p>")
            content = "".join(paragraphs)

        section_html += (
            '<section class="report-section">'
            f'<div class="report-section-kicker">{html.escape(heading)}</div>'
    ...
    return (
        ...
        '<div class="report-chip"><i class="ti ti-file-text"></i> Full report</div>'
        f'<h2>{html.escape(title)}</h2>'
        '<div class="report-subline">Structured summary with cleaned source text and readable evidence sections.</div>'
        '</div>'
        f'<div class="report-sections">{section_html}</div>'
        '</div>'
    )
...
st.markdown(
    ...
    [data-testid="stSidebar"] { border-right: 1px solid var(--b1) !important; }
    ...
    .user-card {
        ...
        padding: 12px 14px; border-top: 1px solid var(--b1); cursor: pointer;
    }
    .avatar {
    ...
    .topbar {
        height: 52px; border-bottom: 1px solid var(--b1);
        display: flex; align-items: center;
        padding: 0 20px; gap: 10px; background: var(--s1);
    }
    ...
    .msg.ai .msg-text {
        background: var(--s2); border: 1px solid var(--b1);
        border-bottom-left-radius: 4px; color: #d8daea;
    }
    ...
    .stSelectbox label {
        color: var(--ink3) !important; font-size: 10.5px !important;
    ...
    .report-hero {
        ...
        background: linear-gradient(135deg, rgba(124,106,255,0.18), rgba(63,252,184,0.07));
    }
    .report-chip {
        ...
        color: var(--a2); font-size: 11px; font-family: 'Geist Mono', monospace;
        text-transform: uppercase; letter-spacing: 0.08em;
    }
    .report-hero h2 {
        margin: 0; color: #f4f5ff;
    ...
    .report-section-body p { margin: 0 0 10px; }
    ...
    .report-section-body a { color: var(--a2); text-decoration: none; border-bottom: 1px solid rgba(168,151,255,0.45); }
    .report-section-body a:hover { color: var(--g); border-bottom-color: rgba(63,252,184,0.6); }

    .status-bar {
        display: flex; align-items: center; gap: 12px;
        padding: 6px 20px; border-top: 1px solid var(--b1);
        background: var(--s1); font-size: 10.5px; color: var(--ink3);
        font-family: 'Geist Mono', monospace;
    }
    .status-dot {
    ...
    for msg in messages:
        ...
        if role == "user":
            ...
            if error:
                ...
                # Full report — auto-open for latest message only
...
# ── Process send ──────────────────────────────────────────────────────────────
MAX_RETRIES = 3   # kitni baar retry kare
RETRY_DELAY = 5   # seconds between each retry
...
if send and user_input.strip():
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        if attempt == 1:
            spinner_msg = "Four agents working — search → read → write → critique…"
        else:
            spinner_msg = f"Pipeline warming up, retrying… ({attempt}/{MAX_RETRIES})"
        ...
        with st.spinner(spinner_msg):
            try:
                ...
                success = True
                break                        # success — loop band karo

            except Exception as exc:
                last_exc = exc
                if attempt < MAX_RETRIES:
                    time.sleep(RETRY_DELAY)  # thoda ruko, phir retry

    # Sirf tab error dikhao jab teeno retries fail hon
    if not success:
        add_message(
            "assistant",
            {
